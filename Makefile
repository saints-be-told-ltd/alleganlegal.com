...
# Project settings
PROJECT := alleganlegal.com
PACKAGE := flask-server/alleganlegal
REPOSITORY := saints-be-told-ltd/alleganlegal.com
...
.PHONY: backup_db
backup_db:
	pg_dump -h localhost -p 5432 -U postgres -d APP -w --data-only --inserts -f database/backup.sql

.PHONY: compile
compile:
	@ echo "Nothing will happen if localhost:5000 isn't available."
	@ grep -q '200 OK' <<< $$(curl -Is http://localhost:5000 | head -1)
	cd flask-server && $(APP) freeze -e publish
